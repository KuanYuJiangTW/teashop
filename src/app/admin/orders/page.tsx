          ...
          <div key={order.id} className="border rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-right">
                <p className="font-medium">NT$ {(order.total / 100).toFixed(0)}</p>
...
function StatusUpdateForm({ orderId, currentStatus }: { orderId: string; currentStatus: OrderStatus }) {
  return (
    <form action={`/api/admin/orders/${orderId}/status`} method="POST" className="flex gap-2">
      <select name="status" defaultValue={currentStatus} className="border rounded px-2 py-1 text-sm">
